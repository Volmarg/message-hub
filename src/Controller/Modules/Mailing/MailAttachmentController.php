...
class MailAttachmentController
{
    ...
    /**
     * Will:
     * - create the attachments physically on the server,
     * - create {@see MailAttachment} entity for each attached file,
     *
     * @param Mail    $mailEntity
     * @param MailDTO $mailDto
     * @param string  $attachmentFolderAbsolutePath
     * @param string  $attachmentFolderRelativePath
     *
     * @return Mail - with the {@see MailAttachment} entities added to it
     * @throws Exception
     */
    public function buildAttachmentsForMail(
        Mail    $mailEntity,
        MailDto $mailDto,
        string  $attachmentFolderAbsolutePath,
        string  $attachmentFolderRelativePath
    ): Mail {
        foreach ($mailDto->getAttachments() as $fileName => $fileContent) {
            $filePath = $attachmentFolderAbsolutePath. DIRECTORY_SEPARATOR . $fileName;

            $isDirCreated = @mkdir($attachmentFolderAbsolutePath, 0755, true);
            if (!$isDirCreated) {
                throw new Exception("Could not created folder for E-Mail attachments: {$attachmentFolderAbsolutePath}, error: " . json_encode(error_get_last()));
            }
            ...
            $attachmentEntity = new MailAttachment($attachmentFolderRelativePath, $fileName, (new File($filePath))->getExtension());
            $mailEntity->addAttachment($attachmentEntity);
        }
