...
{
    protected static $defaultName = 'mh:assets:build-frontend-translation-file';
    ...
    {
        $this->logger->info("Started getting translation files data");

        $translationsAssetsDirectoryExist = file_exists(self::TRANSLATION_BACKEND_FOLDER);
        if( !$translationsAssetsDirectoryExist ){
            $this->logger->critical("Translations assets directory does not exist: ",[
        ...
        foreach( $finder->files() as $file ){
            ...
            $this->logger->info("Found file ({$translationFile}) for language ({$groupName})");
    ...
    {
        $this->logger->info("Started building output messages file");
        ...
        $this->logger->info("Finished building output messages file");
    ...
    private function validateOutputFrontendTranslationFile(): void
    {
        ...
        if( JSON_ERROR_NONE !== json_last_error() ){
            ...
            $this->logger->critical(json_last_error_msg());
        }
