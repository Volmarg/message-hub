...
export default {
  ...
  methods: {
    ...
    searchForStringInTableCells(dataArrayToSearchIn = null){
      ...
      let safeSearchString = this.searchInput.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&');
      let searchRegexp     = new RegExp(safeSearchString, 'i');
      ...
      dataArrayToSearchIn.forEach( (objectWithData) => {
        ...
        // iterate over all props / values of object
        for (let propertyName of this.searchAbleProperties) {
          let value = objectWithData[propertyName];
          ...
          if( "object" === typeof value ){
            ...
            for(let innerObjectValue of value){
              ...
              // object inside object is not allowed to search into - most likely just options / actions column
              if ("object" === typeof innerObjectValue) {
                continue;
              }
