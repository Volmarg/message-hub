        ...
        <section class="">
          ...
          <form ref="form" method="POST" @submit.prevent="submitNewMailAccount">
              ...
              <div class="col-md-10 mb-3">
                <div>
                  ...
                  >
                    <option value=""></option>
                      <option
                        v-for="(clientName, index) in clients"
                        :key="index"
                        :value="clientName">{{ clientName }}</option>
                  </select>
            ...
            <div class="row">
              <div class="col-md-10 mb-3">
                <div>
                  <label for="add_mail_host_client">{{ hostLabelTranslated }}</label>
                  <input class="form-control"
                         id="add_mail_host_client"
                         type="text"
                         required=""
                         :placeholder="hostPlaceholderTranslated"
                         ref="host"
                  >
                </div>
              </div>
...
import GetSupportedMailClientsResponseDto from '../../../../core/dto/api/internal/mailing/GetSupportedMailClientsResponseDto';
...
export default {
  ...
  computed: {
    ...
    hostLabelTranslated: function(){
      return translationsService.getTranslationForString('forms.addMailAccountForm.host.label');
    },
    hostPlaceholderTranslated: function(){
      return translationsService.getTranslationForString('forms.addMailAccountForm.host.placeholder');
    },
  ...
  methods: {
    ...
    getAllMailAccounts(){
      ...
      this.axios.get(SymfonyRoutes.GET_SUPPORTED_MAIL_CLIENTS).then( (response) => {
        this.isSpinnerVisible = false;

        let supportedMailClientsResponse = GetSupportedMailClientsResponseDto.fromAxiosResponse(response);
        this.clients                     = supportedMailClientsResponse.clients;
      });
