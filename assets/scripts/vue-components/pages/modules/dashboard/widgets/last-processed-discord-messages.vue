    ...
    <template #card-body>
      <div v-if="lastProcessedDiscordMessages.length">
        ...
        >
          ...
          <template #actions>
            <Preview :content="buildPreviewContent(discordMessage)"
                     :show-dialog-confirmation-button="false"
            />
          </template>
        </row-fontawesome-icon-with-text>
...
import Preview                             from '../../../../other/actions/preview';
...
export default {
  ...
  methods: {
     ...
     * @returns {string}
     */
    buildPreviewContent(discordMessage){
      let content = `
